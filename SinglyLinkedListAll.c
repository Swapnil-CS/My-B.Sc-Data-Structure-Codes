...
void count_occurrence()
{
	...
	{
		printf("\nEnter a value whose occurrence you want to find:");
		scanf("%d",&key);
		p=start;
		while(p!=NULL)
		{
		...
		if(count==0)
		{
			printf("\nThe value %d is not present in the list",key);
		}
		else
		{
			printf("\nThe value %d has occurred %d times in the list",key,count);
			printf("\nThe addresses of the found node values in new linked list are:");
			s=head;
			while(s!=NULL)
			{
				printf("\t%d",s->data);

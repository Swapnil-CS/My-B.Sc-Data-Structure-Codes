...
void printDLL(struct node *start)
{
	struct node *p;
	p=start;
	printf("\nElements of sorted doubly linked list in ascending order:\n");
	while(p!=NULL)
	{
		printf("\t%d",p->data);
		p=p->next;
...
void main()
{
	...
	while(1)
	{
		printf("\n1.Insertion(randomly enetered values will be sorted internally after insertion)\n2.Display the sorted linked list\n3.Mid value of the sorted linked list\n4.Exit");
		printf("\nEnter your choice:");
		scanf("%d",&ch);
		switch(ch)
		{
			case 1:
				printf("\nEnter a node value which you want to insert:");
				scanf("%d",&item);
